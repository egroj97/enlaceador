...
static QString readRegString(HKEY hKey, const wchar_t* path, const wchar_t* valueName) {
    HKEY key = nullptr;
    if (RegOpenKeyExW(hKey, path, 0, KEY_READ, &key) != ERROR_SUCCESS) return QString();
...
QVector<Browser> BrowserDetector::getInstalledBrowsers() {
    QVector<Browser> browsers;
    QSet<QString> seenPaths;

    auto scanKey = [&](HKEY hive, const wchar_t* basePath) {
        HKEY clientsKey = nullptr;
        if (RegOpenKeyExW(hive, basePath, 0, KEY_READ, &clientsKey) != ERROR_SUCCESS) return;

        wchar_t subkeyName[256];
        DWORD nameLen = 256;
        for (DWORD i = 0; RegEnumKeyExW(clientsKey, i, subkeyName, &nameLen,
            nullptr, nullptr, nullptr, nullptr) == ERROR_SUCCESS; ++i, nameLen = 256) {

            std::wstring subkeyPath = std::wstring(basePath) + L"\\" + subkeyName;
            QString displayName = readRegString(hive, subkeyPath.c_str(), nullptr);
            if (displayName.isEmpty()) displayName = QString::fromWCharArray(subkeyName);
                    ...
                    if (end != -1) {
                        ...
                        if (!seenPaths.contains(exePath)) {
                            ...
                            if (fi.exists() && fi.isFile()) {
                                ...
                                browsers.append({displayName, exePath});
                            }
                        }
                    }
                }
    ...
    CommonPath paths[] = {
        ...
        {"PROGRAMFILES(X86)", "Google\\Chrome\\Application\\chrome.exe", "Google Chrome"},
        {"PROGRAMFILES(X86)", "Mozilla Firefox\\firefox.exe", "Mozilla Firefox"},
        {"PROGRAMFILES(X86)", "Microsoft\\Edge\\Application\\msedge.exe", "Microsoft Edge"},
        {"LOCALAPPDATA", "Google\\Chrome\\Application\\chrome.exe", "Google Chrome"},
        {"LOCALAPPDATA", "Microsoft\\Edge\\Application\\msedge.exe", "Microsoft Edge"},
    };
    ...
    for (const auto& p : paths) {
        ...
        if (!seenPaths.contains(fullPath)) {
            ...
            if (fi.exists() && fi.isFile()) {
                ...
                browsers.append({QString::fromLatin1(p.name), fullPath});
            }
        }
    }
